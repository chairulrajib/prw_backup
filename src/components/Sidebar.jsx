...
const menuItems = {
  false: [
    ...
    { label: "My Booking", icon: FaRegCalendar, url: "/" },
    { label: "Profile", icon: FaUser, url: "/" },
    { label: "Change Profile Picture", icon: FaCamera, url: "/" },
    { label: "Change Password", icon: FaLock, url: "/changepass" },
    { label: "To Be Tenant", icon: FaLock, url: "/tobetenant" },
    // { label: "Logout", icon: FaSignOutAlt, onClick: () => dispatch(logoutAction()) }
  ],
  true: [
    { label: "Home", icon: FaLock, url: "/" },
    { label: "My Booking", icon: FaRegCalendar, url: "/" },
...
const Sidebar = () => {
  ...
  const { isTenant } = useSelector((state) => {
    return {
      isTenant: state.userReducer.isTenant === "true",
    };
  });
